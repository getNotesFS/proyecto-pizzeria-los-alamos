  ...
  ingredientes.create(
    ...
    (err, objetoIngrediente) => {
      if (err) {
        ...
        res.status(201).json(objetoIngrediente);
      }
    }
  );
};
...
const ingredienteList = (req, res) => {
  ingredientes //nombre del modelo
    ...
    .exec((err, objetoIngrediente) => {
      ...
      res.status(200).json(objetoIngrediente);
    });
};
...
const ingredienteUpdate = (req, res) => {
  if (!req.params.ingredienteid) {
    return res
      ...
      .json({
        Mensaje:
          "El ID Ingrediente ingresado no existe, ingrese un ID INGREDIENTE válido.",
      });
  }
  ...
  ingredientes
    ...
    .exec((err, objetoIngrediente) => {
      ...
      objetoIngrediente.save((err, Ingrediente) => {
        if (err) {
...
const ingredienteDelete = (req, res) => {
  if (req.params.ingredienteid) {
    ...
    res.status(404).json({ Mensaje: "Ingredientes no encontrado" });
  }
};
