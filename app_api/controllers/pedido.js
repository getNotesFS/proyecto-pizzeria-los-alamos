...
//Controladores
const pedidoCreate = async (req, res) => {
  pedidos.create(
    {
    ...
    (err, objetoPedido) => {
      if (err) {
        ...
        res.status(201).json(objetoPedido);
      }
    }
  );
};
...
const pedidoList = (req, res) => {
  pedidos //nombre del modelo
    ...
    .exec((err, objetoPedido) => {
      ...
      res.status(200).json(objetoPedido);
    });
};
  ...
  pedidos //nombre del modelo
    ...
    .exec((err, objetoPedido) => {
      if (!objetoPedido) {
        ...
        console.log(
          `Se encontro un error en la Pedido con el id: ${req.params.pedidoid}`
        );
        return res.status(404).json(err);
      }
      console.log(
        `Se encontro el documento Pedido con el id: ${req.params.pedidoid}`
      );
      res.status(200).json(objetoPedido);
    });
};
...
const pedidoUpdate = (req, res) => {
  if (!req.params.pedidoid) {
    return res
      ...
      .json({
        Mensaje:
          "El ID Pedido ingresado no existe, ingrese un ID PEDIDO válido.",
      });
  }

  pedidos.findById(req.params.pedidoid).exec((err, objetoPedido) => {
    if (!objetoPedido) {
      return res.status(404).json({ Mensaje: "El ID Pedido no encontrado." });
    }
    ...
    objetoPedido.save((err, objPedido) => {
      if (err) {
...
const pedidoDelete = (req, res) => {
  if (req.params.pedidoid) {
    pedidos.findByIdAndDelete(req.params.pedidoid).exec((err, objetoPedido) => {
      if (err) {
        return res.status(404).json(err);
      }
